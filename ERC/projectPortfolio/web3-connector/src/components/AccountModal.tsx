...
import { DynamicWalletConfig } from '@/types';
import { useDynamicWallets } from '@/hooks/useDynamicWallets';

interface AccountModalProps {
  isOpen: boolean;
  onClose: () => void;
  walletConnectors: DynamicWalletConfig[];
}
...
export default function AccountModal({ isOpen, onClose ,walletConnectors}: AccountModalProps) {
  ...
  const [selectedConnector, setSelectedConnector] = useState<any>(null);
   const { connectWallet } = useDynamicWallets();

  // 处理钱包连接
  const handleConnect =async (wallet: DynamicWalletConfig) => {
